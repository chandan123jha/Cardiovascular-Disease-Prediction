...
# database code
db = sqlite3.connect(os.path.join(base_dir, 'evaluation.db'))
cursor = db.cursor()
...
def insert():
    ...
    #     ms.showinfo("Message", "password must contain atleast 1 Uppercase letter,1 symbol,1 number")
        ...
        conn = sqlite3.connect(os.path.join(base_dir, 'evaluation.db'))
        with conn:
            cursor = conn.cursor()
            cursor.execute(
...
image2 = Image.open(os.path.join(base_dir, 'images', 'reg1.jpg'))
